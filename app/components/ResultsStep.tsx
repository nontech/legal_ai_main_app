...
export default function ResultsStep({
  ...
  // Register Regenerate button in header when on results page (with provider)
  useEffect(() => {
    if (!setHeaderActions || showGamePlanOnly) return;
    if (!result || !isAuthenticated || !isOwner) {
    ...
    const button = (
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-primary-700 to-primary-600 text-white hover:from-primary-800 hover:to-primary-700 rounded-lg transition-all font-medium text-sm shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
        title={isRegenerating ? tResults("regenerating") : tResults("regenerate")}
      >
        {isRegenerating ? (
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- handleRegenerate/tResults from closure
  }, [setHeaderActions, showGamePlanOnly, result, isAuthenticated, isOwner, isRegenerating]);
    ...
    >
      ...
      <div className="text-center mb-8 px-3 sm:px-0">
        ...
        <p className="text-base sm:text-lg text-gray-600 font-medium">
          Comprehensive analysis based on case details, judicial
          patterns, and legal precedents
        </p>
      </div>
      ...
      {result.key_factors &&
        Object.keys(result.key_factors).length > 0 && (
          ...
          >
              ...
              {Object.entries(result.key_factors)
                ...
                .map(([key, value]) => {
                  ...
                  return (
                    <div
                      key={key}
                      className="flex flex-col sm:flex-row sm:items-center p-3 sm:p-4 mb-2 sm:mb-3 bg-gray-50 rounded-xl border border-gray-100 gap-2 sm:gap-4 transition-all hover:bg-gray-100 cursor-pointer"
                    >
                      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 flex-wrap">
                        <p className="font-semibold text-gray-800 m-0 text-sm sm:text-base">
                          {key}
                        </p>
                        <span
                          style={{
                            ...
                            wordBreak: "break-word",
                            flexShrink: 0,
                            boxShadow: `0 2px 8px ${getBadgeColor(
                              String(value)
                            )}33`,
                          }}
                        >
            ...
            {Object.entries(result.key_factors).filter(
              ([key]) => key.toLowerCase() !== "reasoning"
            ).length > 4 && (
      ...
      {result.legal_assessment && (
        ...
        >
          <div className="flex items-center gap-3 mb-4 sm:mb-5">
            <span className="text-xl sm:text-2xl">⚖️</span>
            <h3
              className="text-lg sm:text-xl font-bold m-0"
              style={{ color: "#e67e22" }}
            >
              {isCriminalCase
                ? "Criminal Assessment"
                  ...
                  <div>
                    <div
                      style={{
                        ...
                        color: "#555",
                        whiteSpace: "pre-wrap",
                        maxHeight: expandedBurdenOfProof
                          ? "none"
                          : "80px",
                        overflow: "hidden",
                      }}
              ...
              {Object.entries(
                ...
                .map(([prop, elem]) => {
                  ...
                  const getColor = (p: number) => {
                    ...
                    const isPlaintiff =
                      result.case_analysis?.role === "plaintiff";
                    const isDefendant =
                      result.case_analysis?.role === "defendant";
                  ...
                  >
                    <div
                      style={{
                        ...
                        background:
                          result.legal_assessment.strength_of_evidence
                            .overall_evidence_score > 7
                            ? "#27ae60"
                            : result.legal_assessment
                                .strength_of_evidence
                                .overall_evidence_score > 5
                            ? "#f39c12"
                            : "#e74c3c",
                      }}
                    ></div>
                    ...
                    >
                      ...
                      <span
                        style={{
                          background:
                            result.legal_assessment
                              .strength_of_evidence.witness_support
                              .rating === "Strong"
                              ? "#27ae60"
                              : result.legal_assessment
                                  .strength_of_evidence
                    ...
                    >
                      ...
                      <span
                        style={{
                          background:
                            result.legal_assessment
                              .strength_of_evidence.expert_reports
                              .rating === "Strong"
                              ? "#27ae60"
                              : result.legal_assessment
                                  .strength_of_evidence.expert_reports
                  ...
                  <div style={{ marginBottom: "16px" }}>
                    ...
                    >
                      {(
                        ...
                        .map((defense, idx) => {
                          const { preview, full, isLong } =
                            truncateWords(defense, 25);
                          return (
                            <div key={idx}>
                              <p
                                style={{
              ...
              .map((rec, idx) => {
                if (typeof rec === "object" && rec.title) {
                  ...
                  return (
                    <div
                      key={idx}
                      className="p-3 sm:p-4 mb-3 rounded-lg"
                      style={{
                        background: "#f8f9fa",
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                        ...
                        <span
                          className="self-start px-3 py-1 rounded text-xs font-bold whitespace-nowrap"
                          style={{
          ...
          >
            ...
            {result.precedent_cases.map((precedent, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="p-3 sm:p-4 mb-3"
                  style={{
                    background: "#f8f9fa",
                    border: "1px solid #e0e0e0",
                    borderLeft: `4px solid ${getSimilarityColor(
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                    ...
                    <span
                      className="self-start px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap"
                      style={{
