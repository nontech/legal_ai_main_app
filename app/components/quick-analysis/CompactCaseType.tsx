...
const DEFAULT_CASE_TYPE =
  DEFAULT_CASE_TYPES.find(
    (caseType) => caseType.id === DEFAULT_CASE_TYPE_ID
  ) ?? DEFAULT_CASE_TYPES[0]!;

function getCaseTypeById(id?: string | null, caseTypes?: CaseType[]) {
  if (!id) return undefined;
  return (caseTypes || DEFAULT_CASE_TYPES).find(
    (caseType) => caseType.id === id
  );
}
...
export default function CompactCaseType({
  ...
  const [selectedCaseType, setSelectedCaseType] = useState<CaseType>(
    () => {
      const initialSelection = getCaseTypeById(
        initialCaseTypeId,
        DEFAULT_CASE_TYPES
      );
  ...
  const lastInitialCaseTypeIdRef = useRef<string | undefined>(
    initialCaseTypeId
  );
  ...
  useEffect(() => {
    ...
    if (lastInitialCaseTypeIdRef.current === initialCaseTypeId) {
      ...
      setSelectedCaseType((current) => {
        const currentExists = caseTypes.some(
          (ct) => ct.id === current.id
        );
    ...
    if (initialCaseTypeId) {
      const targetSelection = getCaseTypeById(
        initialCaseTypeId,
        caseTypes
      );
      if (targetSelection) {
