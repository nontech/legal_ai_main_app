...
interface QuickAnalysisFormProps {
	...
	uploadedDocuments?: Record<
		string,
		{ files: Array<{ name: string; address: string }>; summary?: string }
	>;
}
...
export default function QuickAnalysisForm({
	...
	const [tenancyStatus, setTenancyStatus] = useState<"tenant" | "landlord">(
		"tenant",
	);
	...
	useEffect(() => {
		if (uploadedMetadata && Object.keys(uploadedMetadata).length > 0) {
			...
			if (uploadedMetadata.caseDescription)
				setCaseDescription(uploadedMetadata.caseDescription);
			if (uploadedMetadata.jurisdiction)
				setJurisdiction(uploadedMetadata.jurisdiction);
			if (uploadedMetadata.caseType)
				setCaseTypeId(uploadedMetadata.caseType.toLowerCase());
	...
	useEffect(() => {
		console.log(
			"QuickAnalysisForm useEffect - uploadedDocuments prop:",
			uploadedDocuments,
		);
		console.log("QuickAnalysisForm useEffect - caseId:", caseId);
		console.log(
			"QuickAnalysisForm useEffect - Object.keys(uploadedDocuments):",
		...
		if (propKeys.length > 0) {
			...
			console.log(
				"Setting documents from prop - full data:",
				uploadedDocuments,
			);
					...
					if (response.ok) {
						...
						if (data.case_details) {
							...
							Object.keys(data.case_details).forEach((key) => {
								if (
									key !== "_completion_status" &&
									data.case_details[key]?.files
								) {
									docs[key] = {
	...
	const normalizeCategoryForDisplay = (cat: string): DocumentCategory => {
		...
		if (cat === "key_witnesses_and_testimony")
			return "key_witness_and_testimony";
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (caseId) {
				...
				const normalizedCharges = normalizeCharges(
					uploadedMetadata?.charges || [],
					isCriminalCase,
				);
			...
			sessionStorage.setItem("quickAnalysisData", JSON.stringify(formData));
			const msg =
				e instanceof Error
					? e.message
					: "Error analyzing case. Please try again.";
	...
	const handleStreamingComplete = (result: any) => {
		...
		const existingData = JSON.parse(
			sessionStorage.getItem("quickAnalysisData") || "{}",
		);
		sessionStorage.setItem(
			"quickAnalysisData",
			JSON.stringify({
		...
		setTimeout(() => {
			...
			router.push(
				`/${country}/${locale}/case-analysis/${streamingCaseId}/results`,
			);
		}, 2000);
	};
	...
	return (
		<div className="relative">
			...
			<div className="max-w-6xl mx-auto px-3 sm:px-4 pb-32 space-y-6">
				...
				<div className="space-y-6">
					...
					<JurisdictionSection
						...
						hideSaveButton={true}
						showExtractedBadge={
							!!uploadedMetadata?.extractedFields?.jurisdiction
						}
						hideCourtName={true}
					/>
								...
								<div className="relative group">
									<div className="flex items-center gap-1.5 px-2 py-1 bg-primary-50 border border-primary-200 rounded-md shadow-sm cursor-help">
										...
										<span className="text-xs font-medium text-primary-700">
											Extracted from case information
										</span>
									</div>
									{/* Tooltip */}
						...
						<div className="space-y-4">
							...
							<div>
								...
								<p className="text-xs text-ink-500 mt-2">
									Include dates, locations, key events, and circumstances.
					...
					{Object.keys(documentsByCategory).length > 0 &&
						(() => {
							...
							const fileMap = new Map<
								string,
								{ name: string; address: string; categories: string[] }
							>();
							Object.entries(documentsByCategory).forEach(
								([category, docData]) => {
								...
								<div className="bg-surface-000 p-6">
									...
									<div className="space-y-2">
										{allFiles.map((file, idx) => (
											<div
												key={idx}
												className="flex items-center gap-3 p-3 bg-surface-50 rounded-lg border border-border-100 hover:border-border-200 hover:bg-surface-100 transition-colors"
											>
												...
												<div className="flex items-center gap-2 flex-1 min-w-0 flex-wrap">
													...
													{file.categories.map((cat) => {
														const categoryInfo =
															categoryLabels[normalizeCategoryForDisplay(cat)];
														if (!categoryInfo) return null;
														return (
															<span
																key={cat}
																className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full border flex-shrink-0 ${getCategoryColor(categoryInfo.color)}`}
															>
			...
			{/* Fixed Calculate Results Button at Bottom — z-40 above How it works / footer (z-10–30), below Navbar (z-50) */}
			<div className="fixed bottom-0 left-0 right-0 bg-surface-000 border-t border-border-200 shadow-lg z-40">
