...
export function CountryLanguageSelector() {
    ...
    const handleCountryChange = (newCountry: string) => {
        ...
        const newPath = `/${newCountry}/${newLocale}${restOfPath ? '/' + restOfPath : ''}`;
    ...
    const handleLocaleChange = (newLocale: string) => {
        const pathParts = pathname.split('/');
        const restOfPath = pathParts.slice(3).join('/');
        const newPath = `/${currentCountry}/${newLocale}${restOfPath ? '/' + restOfPath : ''}`;
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="relative">
                ...
                >
                    ...
                    <ChevronDown className={`w-3.5 h-3.5 text-ink-400 transition-transform duration-200 ${isCountryOpen ? 'rotate-180' : ''}`} />
                </button>
                        ...
                        <div className="absolute top-full mt-2 right-0 bg-surface-000 rounded-xl shadow-lg border border-border-200 py-1.5 z-20 min-w-[200px] overflow-hidden animate-in fade-in zoom-in-95 duration-100 origin-top-right">
                            ...
                            {COUNTRIES.map(country => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between group ${currentCountry === country.code
                    ...
                    <div className="flex items-center gap-2">
                        {['en', 'de'].map((locale) => (
                            <button
                                ...
                                disabled={isPending}
                                className={`text-sm transition-colors ${currentLocale === locale
